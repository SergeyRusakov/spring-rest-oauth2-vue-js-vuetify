...
public class EmployeeController {
    ...
    @GetMapping("{id}")
    @PreAuthorize("hasAnyRole('ROLE_USER','ROLE_ADMIN')")
    public Employee getById(@PathVariable int id) throws DataNotFoundException {
        return employeeControllerService.getEmployee(id);
    }

    @DeleteMapping("{id}")
    @PreAuthorize("hasAnyRole('ROLE_ADMIN')")
    public void delete(@PathVariable int id) throws DataNotFoundException {
        employeeControllerService.delete(id);
    }

    @PutMapping
    @PreAuthorize("hasAnyRole('ROLE_ADMIN')")
    public Employee update(@RequestBody @Valid Employee newEmployee,
                                   BindingResult bindingResult) throws InvalidDataException {
        if(bindingResult.hasErrors()) throw new InvalidDataException();
        return employeeControllerService.update(newEmployee);
    }
    ...
    public Employee save(@RequestBody @Valid Employee employee,
                         BindingResult bindingResult) throws InvalidDataException {
        if(bindingResult.hasErrors()) throw new InvalidDataException();
        return employeeControllerService.save(employee);
    }
}
